...
When multiple users run test mode at the same time, sharing the default
`dev` namespace can cause collisions in both result folders and cohort
table names.
...
- **Results path namespace** under
  `exec/results/[database]/[testLabel]/...`
- **Cohort table suffix** in execution settings (for example
  `_feature_ml_test`)
...
This behavior is **test mode only**. Production execution remains strict
and uses semantic versioning with no custom suffix overrides.
