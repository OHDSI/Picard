...
| **B. Cohorts “” First Time**      | [`initCohortManifest()`](https://ohdsi.github.io/Picard/reference/initCohortManifest.md), fill CSV, connect ATLAS, `importAtlasCohorts()`                                                                                   |
...
`~/.picard/secrets.yml` using the secrets management system:
...
stored securely. The file should contain an `atlas` section:
...
You can store credentials three ways: - **Plaintext** (not recommended):
`password: "YourPassword"` - **Keyring** (recommended):
`password: !expr keyring::key_get(...)` - **Environment variable**:
`password: !expr Sys.getenv("ATLAS_PASSWORD")`
