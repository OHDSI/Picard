...
dependencies on existing manifest cohorts. The SQL is copied to
`derived/` and the skip-logic handles dependency-aware hashing
automatically.
...
Filters an outcome cohort based on whether a prior target (exposure)
event exists. Optionally restricts the lookback to a time window.
...
Reverse direction — filters a target cohort based on whether a prior
