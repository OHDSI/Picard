...
Merged results can get large once every database's rows for a task are combined.
Pass `compress = TRUE` to write the merged per-task result files as gzip-compressed
`.csv.gz` instead of plain `.csv`:
...
- `readr::write_csv()` gzips automatically when the filename ends in `.csv.gz`, and
  `readr::read_csv()`/`spec_csv()` read `.csv.gz` files transparently on the way back
  in — no special handling needed to consume them, including in `reviewExportSchema()`
  and `validateCohortResults()`.
- Only the merged per-task result files are affected. Reference/QC files
  (`cohortManifestSnapshot.csv`, `databaseInfo.csv`, `schema_review.csv`,
  `qc_cohortValidation.csv`, `qc_processMeta.csv`) are always written as plain `.csv`,
  since those tend to stay small.
- Any script that reads a merged result file directly (e.g. a dissemination script)
  needs to account for the `.csv.gz` extension when `compress = TRUE` was used —
  either check for both extensions, or note in your own documentation which
  convention this study uses.
