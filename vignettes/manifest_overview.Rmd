...
| `status` | `active`, `deleted`, or `purged` |
...
parent hashes plus the `dependency_rule` is used — so changing a parent cohort
or its parameters triggers re-execution of all dependents.
...
start_window <- createSubsetStartWindow(
  subsetCohortWindowAnchor = "cohort start",
  startDays = -365L,
  endDays   = 0L,
  baseCohortWindowAnchor = "cohort start"
)
...
stored hashes and reports any files that appear on disk but are not registered:
