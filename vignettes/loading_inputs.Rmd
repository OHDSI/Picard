...
remaining builder scripts (in the correct dependency order: concept sets first,
then cohorts). This means:
...
| Script | Purpose |
|---|---|
| `importAtlas.R` | Bulk import concept sets from ATLAS via CSV + connection |
...
| Script | Purpose |
|---|---|
| `importAtlas.R` | Bulk import cohorts from ATLAS via CSV + connection |
...
4. **Run `main.R`** — `sourceBuilderScripts()` auto-discovers and runs remaining scripts
...
inputs/conceptSets/R/
  ✓ importAtlas.R
  ✗ importCapr.R (deleted)

inputs/cohorts/R/
  ✗ importAtlas.R (deleted)
  ✓ importCapr.R
  ✗ importSql.R (deleted)
  ✗ buildDependentCohorts.R (deleted)
```

When `main.R` runs, only `importAtlas.R` and `importCapr.R` source (concept sets first, then cohorts).
...
This pattern uses `inputs/conceptSets/R/importAtlas.R`.
...
This pattern uses `inputs/cohorts/R/importAtlas.R`.
...
This pattern uses `inputs/conceptSets/R/importCapr.R` and requires the Capr package.
...
This pattern uses `inputs/cohorts/R/importCapr.R` and requires the Capr package.
...
This pattern uses `inputs/cohorts/R/importSql.R`.
...
This pattern uses `inputs/cohorts/R/buildDependentCohorts.R`.
...
when `main.R` executes `sourceBuilderScripts()`.
